...
# ╔═╡ b5eb2798-6e3f-11ef-25cc-91ffcd2df5d5
using Tidier,PlutoUI

# ╔═╡ cadaa731-3b99-46ab-829f-eb617f6269ad
TableOfContents(title="目录")

# ╔═╡ de84427a-e61d-4a28-a367-a4dbf9f882fc
md"""
## TidierFiles数据读写
只总结常用的4个函数 `read_csv`、`write_csv`、`read_xlsx` 和 `write_xlsx`：

### read_csv
**函数签名:**
```julia
read_csv(file; delim=',', col_names=true, skip=0, n_max=Inf, comment=nothing, missingstring="", col_select, escape_double=true, col_types=nothing, num_threads=1)
```

**参数含义:**
- `file`: CSV 文件的路径或 URL。
- `delim`: 定界符，默认为逗号 `,`。
- `col_names`: 是否使用 CSV 的第一行作为列名，默认为 `true`。
- `skip`: 开始读取数据前要跳过的初始行数，默认为 `0`。
- `n_max`: 最大读取行数，默认为 `Inf`（读取所有行）。
- `comment`: 注释行的起始字符，默认无。
- `missingstring`: 表示缺失值的字符串，默认为空字符串 `""`。
- `col_select`: 选择加载哪些列的可选向量。
- `escape_double`: 是否将连续的双引号解释为单个引号，默认为 `true`。
- `col_types`: 列类型的可选指定，默认为 `nothing`（类型推断）。
- `num_threads`: 并行执行时使用的线程数，默认为 `1`。

**例子:**
```julia
julia> df = read_csv("csvtest.csv")
```

### write_csv
**函数签名:**
```julia
write_csv(df, filepath; na="", append=false, col_names=true, missingstring, eol="\n", num_threads=Threads.nthreads())
```

**参数含义:**
- `df`: 要写入 CSV 文件的 DataFrame。
- `filepath`: 输出 CSV 文件的路径。
- `na`: 表示缺失值的字符串，默认为空字符串 `""`。
- `append`: 是否追加到文件，默认为 `false`。
- `col_names`: 是否写入列名，默认为 `true`。
- `missingstring`: 表示缺失值的字符串，默认为空字符串 `""`。
- `eol`: 输出文件中使用的行结束字符，默认为换行符 `\n`。
- `num_threads`: 写文件时使用的线程数，默认为可用线程数。
...
- `range`: 要读取的单元格范围，默认读取整个工作表。
- `col_names`: 是否将第一行作为列名，默认为 `true`。
- `col_types`: 明确指定列类型，默认为 `nothing`（类型推断）。
- `missingstring`: 表示缺失值的字符串或向量。
- `trim_ws`: 是否去除单元格的前后空格。
- `skip`: 开始读取前要跳过的行数。
- `n_max`: 最大读取行数，默认为 `Inf`（读取所有行）。
- `guess_max`: 用于类型猜测和列名检测的最大行数。

**例子:**
```julia
julia> df = read_xlsx("xlsxtest.xlsx", sheet="Sheet1")
```
...
**参数含义:**
- `x`: 要写入的数据，可以是一个 `Pair`（对于一个工作表），或多个 `Pair` 的元组（对于多个工作表）。
- `path`: Excel 文件的路径。
- `overwrite`: 是否覆盖现有文件，默认为 `false`。

**例子:**
```julia
julia> write_xlsx(("REPORT_A" => df1, "REPORT_B" => df2), path="xlsxtest.xlsx", overwrite=true)
```

这些函数提供了在 Julia 中处理 CSV 和 Excel 文件的强大功能，使得数据导入和导出变得简单高效。
"""

# ╔═╡ 3e8ea521-bf1a-4982-99f7-5462c7c6d109
md"""
## Tidier.jl包中的时间处理函数总结
### 时间判断函数
- **am**
  - **作用**：检查时间是否在上午。
  - **例子**：`julia> am(DateTime(2023, 6, 15, 9, 30, 0))` 返回 `true`。

- **pm**
  - **作用**：检查时间是否在下午。
  - **例子**：`julia> pm(DateTime(2023, 6, 15, 15, 30, 0))` 返回 `true`。

### 月份天数函数
- **days_in_month**
  - **作用**：返回一个月中的天数。
  - **例子**：`julia> days_in_month(Date(2020, 2, 29))` 返回 `29`。

### 时间差计算函数
- **difftime**
  - **作用**：计算两个时间之间的差异。
  - **例子**：`julia> difftime(DateTime(2023, 6, 15, 9, 30, 0), DateTime(2023, 6, 15, 8, 30, 0), "hours")` 返回 `1.0`。

### 日期解析函数
- **dmy**
  - **作用**：将不同格式的日期字符串转换为日期对象。
  - **例子**：`julia> dmy("03122020")` 返回 `2020-12-03`。

- **mdy**
  - **作用**：将不同格式的日期字符串转换为日期对象。
  - **例子**：`julia> mdy("12032020")` 返回 `2020-12-03`。

- **ymd**
  - **作用**：将不同格式的日期字符串转换为日期对象。
  - **例子**：`julia> ymd("20201203")` 返回 `2020-12-03`。

### 日期时间解析函数
- **dmy_h**
  - **作用**：将日期和时间字符串转换为日期时间对象。
  - **例子**：`julia> dmy_h("01/01/2020 4PM")` 返回 `2020-01-01T16:00:00`。

- **dmy_hm**
  - **作用**：将日期和时间字符串转换为日期时间对象。
  - **例子**：`julia> dmy_hm("01/01/2020 4:30PM")` 返回 `2020-01-01T16:30:00`。

- **dmy_hms**
  - **作用**：将日期和时间字符串转换为日期时间对象。
  - **例子**：`julia> dmy_hms("15-06-2023 09:30:00")` 返回 `2023-06-15T09:30:00`。

- **mdy_h**
  - **作用**：将日期和时间字符串转换为日期时间对象。
  - **例子**：`julia> mdy_h("06-15-2023 09hr")` 返回 `2023-06-15T09:00:00`。

- **mdy_hm**
  - **作用**：将日期和时间字符串转换为日期时间对象。
  - **例子**：`julia> mdy_hm("06-15-2023 09:03 P")` 返回 `2023-06-15T21:03:00`。

- **mdy_hms**
  - **作用**：将日期和时间字符串转换为日期时间对象。
  - **例子**：`julia> mdy_hms("06/15/2023 08:30:15")` 返回 `2023-06-15T08:30:15`。

- **ymd_h**
  - **作用**：将日期和时间字符串转换为日期时间对象。
  - **例子**：`julia> ymd_h("2023-06-15 09hr")` 返回 `2023-06-15T09:00:00`。

- **ymd_hm**
  - **作用**：将日期和时间字符串转换为日期时间对象。
  - **例子**：`julia> ymd_hm("2023-06-15 09:30")` 返回 `2023-06-15T09:30:00`。

- **ymd_hms**
  - **作用**：将日期和时间字符串转换为日期时间对象。
  - **例子**：`julia> ymd_hms("2023-06-15 09:30:00")` 返回 `2023-06-15T09:30:00`。

### 时间处理函数
- **floor_date**
  - **作用**：向下舍入日期时间对象到指定单位。
  - **例子**：`julia> floor_date(DateTime(2023, 6, 15, 9, 45), "hour")` 返回 `2023-06-15T09:00:00`。

- **round_date**
  - **作用**：四舍五入日期时间对象到指定单位。
  - **例子**：`julia> round_date(DateTime(2023, 6, 15, 9, 45), "hour")` 返回 `2023-06-15T10:00:00`。

### 闰年判断函数
- **leap_year**
  - **作用**：检查年份是否为闰年。
  - **例子**：`julia> leap_year(2020)` 返回 `true`。

### 时间获取函数
- **now**
  - **作用**：获取当前日期和时间。
  - **例子**：`julia> now()` 返回当前日期和时间。

- **today**
  - **作用**：获取当前日期。
  - **例子**：`julia> today()` 返回当前日期。

### 时间解析函数
- **hms**
  - **作用**：将时间字符串转换为时间对象。
  - **例子**：`julia> hms("12:34:56")` 返回 `12:34:56`。
"""

# ╔═╡ c392d0d7-9292-4980-8b97-3376cb602b1f
md"""
## Dates包中的日期时间处理函数
这个是Julia自带的包， 不需要安装直接可以`using Dates`之后使用
### 时间类型与周期构造函数
- **DateTime**
  - **作用**：构造一个DateTime类型的日期时间。
  - **例子**：`julia> DateTime(2023, 6, 15, 9, 30)` 返回 `2023-06-15T09:30:00`。

- **Date**
  - **作用**：构造一个Date类型的日期。
  - **例子**：`julia> Date(2023, 6, 15)` 返回 `2023-06-15`。

- **Time**
  - **作用**：构造一个Time类型的具体时间。
  - **例子**：`julia> Time(9, 30)` 返回 `09:30:00`。

### 时间解析函数
- **parse**
  - **作用**：解析字符串为Date或DateTime类型。
  - **例子**：`julia> parse(Date, "2023-06-15")` 返回 `2023-06-15`。

- **tryparse**
  - **作用**：尝试解析字符串为Date或DateTime类型，失败时返回nothing。
  - **例子**：`julia> tryparse(DateTime, "2023-06-15T09:30:00")` 返回 `DateTime("2023-06-15T09:30:00")`。

### 时间格式函数
- **format**
  - **作用**：将日期时间格式化为字符串。
  - **例子**：`julia> format(DateTime(2023, 6, 15, 9, 30), "yyyy-MM-dd HH:mm:ss")` 返回 `"2023-06-15 09:30:00"`。

### 时间访问函数
- **year**
  - **作用**：获取日期时间的年份部分。
  - **例子**：`julia> year(DateTime(2023, 6, 15))` 返回 `2023`。

- **month**
  - **作用**：获取日期时间的月份部分。
  - **例子**：`julia> month(Date(2023, 6, 15))` 返回 `6`。

- **day**
  - **作用**：获取日期时间的日份部分。
  - **例子**：`julia> day(Date(2023, 6, 15))` 返回 `15`。

- **hour**
  - **作用**：获取日期时间的小时部分。
  - **例子**：`julia> hour(DateTime(2023, 6, 15, 9, 30))` 返回 `9`。

- **minute**
  - **作用**：获取日期时间的分钟部分。
  - **例子**：`julia> minute(DateTime(2023, 6, 15, 9, 30))` 返回 `30`。

- **second**
  - **作用**：获取日期时间的秒份部分。
  - **例子**：`julia> second(DateTime(2023, 6, 15, 9, 30, 15))` 返回 `15`。

### 时间比较函数
- **isless**
  - **作用**：判断一个日期时间是否早于另一个。
  - **例子**：`julia> isless(DateTime(2023, 6, 15), DateTime(2024, 6, 15))` 返回 `true`。

- **isgreater**
  - **作用**：判断一个日期时间是否晚于另一个。
  - **例子**：`julia> isgreater(DateTime(2024, 6, 15), DateTime(2023, 6, 15))` 返回 `true`。

### 时间算术函数
- **add_years**
  - **作用**：给日期时间加上指定年数。
  - **例子**：`julia> add_years(Date(2023, 6, 15), 1)` 返回 `2024-06-15`。

- **add_months**
  - **作用**：给日期时间加上指定月数。
  - **例子**：`julia> add_months(Date(2023, 6, 15), 1)` 返回 `2023-07-15`。

- **add_days**
  - **作用**：给日期时间加上指定天数。
  - **例子**：`julia> add_days(Date(2023, 6, 15), 1)` 返回 `2023-06-16`。

- **add_hours**
  - **作用**：给日期时间加上指定小时数。
  - **例子**：`julia> add_hours(DateTime(2023, 6, 15, 9, 30), 2)` 返回 `2023-06-15T11:30:00`。

### 时间周期函数
- **days**
  - **作用**：创建一个Days周期对象。
  - **例子**：`julia> days(1)` 返回 `1 day`。

- **hours**
  - **作用**：创建一个Hours周期对象。
  - **例子**：`julia> hours(2)` 返回 `2 hours`。

- **minutes**
  - **作用**：创建一个Minutes周期对象。
  - **例子**：`julia> minutes(30)` 返回 `30 minutes`。

### 时间区间函数
- **range**
  - **作用**：创建一个时间区间。
  - **例子**：`julia> range(Date(2023, 6, 15), stop=Date(2023, 6, 20), step=Day(1))` 返回 `2023-06-15:Day(1):2023-06-20`。

"""

# ╔═╡ 8516d0d7-2251-4b92-b9d6-cb2344181f51
md"""
## TidierStrings包中的字符串处理函数
总结了以下数量的函数：
- 匹配函数：12个
- 连接函数：3个
- 字符函数：7个
...
### 匹配函数
- **str_count**
  - **作用**：计算字符串中某个模式的非重叠出现次数。
  - **例子**：`julia> str_count("The blue sky is blue", "blue")` 返回 `2`。

- **str_detect**
  - **作用**：确定字符串是否包含特定模式。
  - **例子**：`julia> str_detect("The sky is blue", "blue")` 返回 `true`。

- **str_locate**
  - **作用**：返回字符串中某个模式首次出现的索引。
  - **例子**：`julia> str_locate("apple", "e")` 返回 `(5, 5)`。

- **str_locate_all**
  - **作用**：返回字符串中某个模式所有出现的索引。
  - **例子**：`julia> str_locate_all("apple", "e")` 返回 `[(5, 5)]`。

- **str_replace**
  - **作用**：替换字符串中首次出现的指定模式。
  - **例子**：`julia> str_replace("The sky is blue", "blue", "red")` 返回 `"The sky is red"`。

- **str_replace_all**
  - **作用**：替换字符串中所有出现的指定模式。
  - **例子**：`julia> str_replace_all("The blue sky is blue", "blue", "red")` 返回 `"The red sky is red"`。

- **str_remove**
  - **作用**：移除字符串中首次出现的指定模式。
  - **例子**：`julia> str_remove("I love tidier strings", " strings")` 返回 `"I love tidier"`。

- **str_remove_all**
  - **作用**：移除字符串中所有出现的指定模式。
  - **例子**：`julia> str_remove_all("I love tidier strings, I love tidier strings", " strings")` 返回 `"I love tidier, I love tidier"`。

- **str_starts**
  - **作用**：检查字符串是否以特定模式开始。
  - **例子**：`julia> str_starts("apple pineapple", "^p")` 返回 `false`。

- **str_ends**
  - **作用**：检查字符串是否以特定模式结束。
  - **例子**：`julia> str_ends("apple pineapple", "e$")` 返回 `false`。

- **str_subset**
  - **作用**：基于字符串中是否存在特定模式来提取子集。
  - **例子**：`julia> str_subset("Hello world!", "world")` 返回 `"Hello world!"`。

- **str_which**
  - **作用**：返回字符串中至少有一次匹配到模式的索引。
  - **例子**：`julia> str_which(["apple", "banana", "pear", "pineapple"], "a")` 返回 `[1, 2, 3, 4]`。

### 连接函数
- **str_c**
  - **作用**：将字符串数组连接成一个字符串。
  - **例子**：`julia> str_c(["apple", "banana", "pear", "pineapple"])` 返回 `"applebananapearpineapple"`。

- **str_flatten**
  - **作用**：将字符串数组合并为一个字符串。
  - **例子**：`julia> str_flatten(["a", "b", "c"])` 返回 `"abc"`。

- **str_flatten_comma**
  - **作用**：将字符串数组用逗号分隔并合并成一个字符串。
  - **例子**：`julia> str_flatten_comma(['a', 'b', 'c'])` 返回 `"a, b, c"`。

### 字符函数
- **str_dup**
  - **作用**：重复字符串多次。
  - **例子**：`julia> str_dup("hello", 3)` 返回 `"hellohellohello"`。

- **str_length**
  - **作用**：返回字符串的长度。
  - **例子**：`julia> str_length("hello world! 😊")` 返回 `14`。

- **str_width**
  - **作用**：返回字符串的宽度。
  - **例子**：`julia> str_width("hello world! 😊")` 返回 `15`。

- **str_trim**
  - **作用**：去除字符串两侧的空白字符。
  - **例子**：`julia> str_trim("  hello world!  ")` 返回 `"hello world!"`。

- **str_squish**
  - **作用**：压缩字符串中的连续空白字符，并去除首尾空白。
  - **例子**：`julia> str_squish("  This    is a string   with   spaces   ")` 返回 `"This is a string with spaces"`。

- **str_wrap**
  - **作用**：根据给定宽度自动换行字符串。
  - **例子**：`julia> str_wrap("This is an example text that should be wrapped based on the given width and breaking rules.", width=20)` 返回 `"This is an example\ntext that should be\nwrapped based on the\ngiven width and\nbreaking rules."`。

- **str_pad**
  - **作用**：对字符串进行填充以达到特定宽度。
  - **例子**：`julia> str_pad("hello", 10)` 返回 `"hello     "`。

### 语言环境函数
- **str_equal**
  - **作用**：检查字符串是否完全等于某个模式。
  - **例子**：`julia> str_equal("hello", "hello")` 返回 `true`。

- **str_to_upper**
  - **作用**：将字符串中的所有字符转换为大写。
  - **例子**：`julia> str_to_upper("Hello World!")` 返回 `"HELLO WORLD!"`。

- **str_to_lower**
  - **作用**：将字符串中的所有字符转换为小写。
  - **例子**：`julia> str_to_lower("Hello World!")` 返回 `"hello world!"`。

- **str_to_title**
  - **作用**：将字符串中每个单词的首字母转换为大写。
  - **例子**：`julia> str_to_title("hello world!")` 返回 `"Hello World!"`。

- **str_to_sentence**
  - **作用**：将字符串中每个句子的首字母转换为大写。
  - **例子**：`julia> str_to_sentence("hello world!")` 返回 `"Hello world!"`。

- **str_unique**
  - **作用**：从字符串数组中移除重复项。
  - **例子**：`julia> str_unique(["apple", "banana", "pear", "banana", "Apple"])` 返回 `["apple", "banana", "pear", "Apple"]`。

### 其他函数
- **str_conv**
  - **作用**：将字符串转换为不同的编码。
  - **例子**：`julia> str_conv("Hello, World!", "UTF-8")` 返回 `"Hello, World!"`。

- **str_like**
  - **作用**：使用SQL样式的模式匹配来检测字符串中的模式。
  - **例子**：`julia> str_like("hello", "h_llo")` 返回 `true`。

- **str_replace_missing**
  - **作用**：将字符串数组中的缺失值替换为指定字符串。
  - **例子**：`julia> str_replace_missing(["apple", missing, "pear", "pineapple"])` 返回 `["apple", "missing", "pear", "pineapple"]`。

- **word**
  - **作用**：从字符串中提取单词。
  - **例子**：`julia> word("Jane saw a cat", 1)` 返回 `["Jane"]`。
"""
...
### 数据预处理函数
- **as_float**
  - **作用**：将数字或字符串转换为Float64数据类型。
  - **例子**：`as_float("1.5")` 返回 `1.5`。

- **as_integer**
  - **作用**：将数字或字符串转换为Int64数据类型。
  - **例子**：`as_integer("2.5")` 返回 `2`。

- **as_string**
  - **作用**：将数字或字符串转换为String数据类型。
  - **例子**：`as_string(1)` 返回 `"1"`。

- **missing_if**
  - **作用**：将特定的值替换为缺失值。
  - **例子**：`missing_if(x, 4)` 如果x是4，则返回`missing`。

- **replace_missing**
  - **作用**：将缺失值替换为指定的替换值。
  - **例子**：`replace_missing(x, 100)` 如果x是`missing`，则返回`100`。

### 数据筛选函数
- **@filter**
  - **作用**：根据指定条件筛选子集。
  - **例子**：`@filter(df, b >= mean(b))` 返回均值`b`以上的行。

- **@slice**
  - **作用**：通过索引选择、移除或复制行。
  - **例子**：`@slice(df, 1:5)` 返回数据框的前5行。

- **@drop_missing**
  - **作用**：删除所有包含缺失值的行。
  - **例子**：`@drop_missing(df)` 返回不含缺失值的行。

### 数据排序函数
- **@arrange**
  - **作用**：根据指定列的值对数据框的行进行排序。
  - **例子**：`@arrange(df, a, desc(b))` 按列`a`升序和列`b`降序排列。

- **desc**
  - **作用**：在`@arrange()`中使用，以降序排序列。
  - **例子**：`desc(b)` 与`@arrange()`结合使用，对列`b`降序排序。

### 数据聚合函数
- **@summarize**
  - **作用**：创建一个包含所有观测值汇总的新数据框。
  - **例子**：`@summarize(df, mean_b = mean(b))` 返回列`b`的均值。

- **@count**
  - **作用**：统计一个或多个变量的唯一值。
  - **例子**：`@count(df, a)` 返回列`a`的唯一值计数。

### 数据变换函数
- **@mutate**
  - **作用**：创建新列或更改现有列的值。
  - **例子**：`@mutate(df, d = b + c)` 创建新列`d`，值为`b`和`c`的和。

- **across**
  - **作用**：在多个变量上应用函数。
  - **例子**：`@summarize(df, across(b:c, minimum))` 计算列`b`到`c`的最小值。

### 数据连接函数
- **@inner_join**
  - **作用**：执行两个数据框的内连接。
  - **例子**：`@inner_join(df1, df2, a)` 按列`a`内连接`df1`和`df2`。

- **@left_join**
  - **作用**：执行两个数据框的左连接。
  - **例子**：`@left_join(df1, df2, a)` 按列`a`左连接`df1`和`df2`。

- **@right_join**
  - **作用**：执行两个数据框的右连接。
  - **例子**：`@right_join(df1, df2, a)` 按列`a`右连接`df1`和`df2`。

- **@full_join**
  - **作用**：执行两个数据框的全连接。
  - **例子**：`@full_join(df1, df2, a)` 按列`a`全连接`df1`和`df2`。

- **@anti_join**
  - **作用**：执行两个数据框的反对接。
  - **例子**：`@anti_join(df1, df2, a)` 按列`a`反对接`df1`和`df2`。

### 数据重塑函数
- **@pivot_longer**
  - **作用**：将宽格式数据框重塑为长格式。
  - **例子**：`@pivot_longer(df, A:B)` 将列`A`到`B`的数据变为长格式。

- **@pivot_wider**
  - **作用**：将长格式数据框重塑为宽格式。
  - **例子**：`@pivot_wider(df, names_from = variable, values_from = value)` 将数据从长格式变为宽格式。

### 数据选择函数
- **@select**
  - **作用**：选择数据框中的变量。
  - **例子**：`@select(df, a, b)` 选择列`a`和`b`。

- **@rename**
  - **作用**：更改数据框中列的名称。
  - **例子**：`@rename(df, d = b)` 将列`b`重命名为`d`。

### 数据分组函数
- **@group_by**
  - **作用**：根据一个或多个列对数据框进行分组。
  - **例子**：`@group_by(df, a)` 按列`a`分组。

- **@ungroup**
  - **作用**：移除数据框中的所有分组。
  - **例子**：`@ungroup(df)` 移除`df`的所有分组。

### 数据汇总函数
- **@summary**
  - **作用**：为数值列提供汇总统计。
  - **例子**：`@summary(df, b:d)` 提供列`b`到`d`的汇总统计。

### 数据抽样函数
- **@slice_sample**
  - **作用**：从数据框中随机抽取行。
  - **例子**：`@slice_sample(df, n = 5)` 随机抽取5行。

- **@slice_head**
  - **作用**：从数据框的开始处获取行。
  - **例子**：`@slice_head(df, n = 3)` 获取数据框的前3行。

- **@slice_tail**
  - **作用**：从数据框的末尾处获取行。
  - **例子**：`@slice_tail(df, n = 3)` 获取数据框的后3行。

- **@slice_max**
  - **作用**：获取指定列具有最大值的行。
  - **例子**：`@slice_max(df, b)` 获取列`b`最大值所在的行。

- **@slice_min**
  - **作用**：获取指定列具有最小值的行。
  - **例子**：`@slice_min(df, b)` 获取列`b`最小值所在的行。

### 数据分解函数
- **@separate**
  - **作用**：根据指定分隔符将字符串列拆分为多个新列。
  - **例子**：`@separate(df, a, [b, c], "-")` 将列`a`按`'-'`拆分为列`b`和`c`。

- **@separate_rows**
  - **作用**：根据给定分隔符将指定列的内容拆分为多行。
  - **例子**：`@separate_rows(df, 2, ";")` 将列`2`按`';'`拆分为多行。

### 数据整理函数
- **@relocate**
  - **作用**：在数据框中重新排列列的位置。
  - **例子**：`@relocate(df, b, before = c)` 将列`b`移动到列`c`之前。

- **@unite**
  - **作用**：将多个列合并为一个新列。
  - **例子**：`@unite(df, new_col, (b, c), "-")` 将列`b`和`c`合并为新列`new_col`。

### 数据嵌套函数
- **@nest**
  - **作用**：将多个列嵌套到一个新的列中，形成数据框。
  - **例子**：`@nest(df, data = b:c)` 将列`b`到`c`嵌套到新列`data`中。

- **@unnest_longer**
  - **作用**：将包含数组的列展开为更长的数据框。
  - **例子**：`@unnest_longer(df, data)` 将列`data`中的数组展开。

- **@unnest_wider**
  - **作用**：将包含数组或字典的列展开为更宽的数据框。
  - **例子**：`@unnest_wider(df, data)` 将列`data`中的数组或字典展开。

### 数据特殊处理函数
- **case_when**
  - **作用**：根据条件返回相应的值。
  - **例子**：`julia> case_when(a > 4  =>  "hi", a > 2  =>  "medium", a > 0  =>  "low")` 根据`a`的值返回不同的字符串。

- **if_else**
  - **作用**：根据条件返回`yes`或`no`值。
  - **例子**：`julia> if_else(a >= 3, "yes", "no")` 如果`a`大于等于3，返回`"yes"`，否则返回`"no"`。

- **n**
  - **作用**：返回数据框或组中的行数。
  - **例子**：`julia> n()` 在`@summarize`中使用，返回行数。

- **ntile**
  - **作用**：将输入向量分成`n`个大小相等的桶。
  - **例子**：`julia> ntile(x, 4)` 将向量`x`分成4个分位数。
...
manifest_format = "2.0"
project_hash = "fd226503868e415efa7d5946a793a92c0c0061b9"
