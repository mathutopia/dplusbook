...
# ╔═╡ 8220135a-df6c-4aae-bd23-404a8eb52e0c
begin
function assignments(clus::Vector{Cluster})
	n = sum(map(c->length(c.id), clus))
	ret = Vector{Int32}(undef, n)
...
assignments(onepass::OnePassResult) = onepass.assignments
counts(onepass::OnePassResult) = onepass.counts
end

# ╔═╡ 20f0c6f1-364a-49c7-810c-2a20a0c98c13
begin
...
# ╔═╡ 994a4e6c-f6e9-4f52-84bd-85e0df6a926a
X = Matrix(data1[:, 1:4])' # 注意这里的转换为矩阵并转置

# ╔═╡ 09297e86-091e-46a2-af79-fcf73b137615
md"""
下面的代码以阈值r=0.1构建簇，
"""

# ╔═╡ 1857feae-3e6c-4019-90ed-60a247ee2fe1
clu = onepass(X, 100000)

# ╔═╡ b50115ba-685d-4855-81eb-28e7ca4d128a
md"""
由于返回结果是一个簇的向量， 可以通过这个向量方便的获得想要的结果。比如，可以通过`length`函数，获得最终生成的簇的数量。 下面的代码返回每个簇的样本个数。
"""

# ╔═╡ ee29e32b-8372-4d5c-a9b0-5950dcd0485f
md"""
下面的代码可以查看每个样本被分配到哪一个簇中。
"""

# ╔═╡ bd602300-c995-432c-abb6-90ea1c229866
assignments(clu)
...
manifest_format = "2.0"
project_hash = "2216fa7934de9ac8db791189654a0a4d6d8f0ad3"
