...
数据在内存中是以**值（value）**的形式存在的。 如果把代码构建的虚拟世界类比人类世界， 那么值就是这个虚拟世界的各种客观存在（对象）。值的作用是参与运算， 为了方便计算， 很多时候， 我们需要给值取一个名字， 这就是**变量(variable)**的概念。一个变量是一个跟值关联在一起的名字。 通常， 可以用**绑定操作符（=）**将一个值一个变量名关联起来。 
...
5. **关键字作为变量名**：
   - Julia的关键字（如`else`、`try`）不能作为变量名。

6. **赋值与可变类型**：
   - 赋值是创建变量与值之间的绑定，而可变类型（如数组）可以通过索引进行修改。
   ```julia
   a = [1, 2, 3]  # 创建一个数组
   b = a          # b和a指向同一个数组
   a[1] = 42      # 修改数组的第一个元素，b也能看到这个变化
   ```

7. **函数与变量**：
   - 函数参数的赋值类似于变量赋值，参数值的改变不会影响外部变量。
   - 以`!`结尾的函数名通常表示该函数会修改其参数。
...
这些是Julia中变量使用的一些基本规则和约定。通过遵循这些规则，可以编写出清晰、可维护的Julia代码。
"""

# ╔═╡ 385ada41-c293-40d8-a15f-da9d207db46a
md"""
## 对比C++
Julia语言的变量系统与C++有一些显著的不同之处，这些差异主要体现在以下几个方面：
...
# ╔═╡ 39e85750-067a-4d65-a620-a763f1b16080
md"""
在代码示例中， 

- #符号后面内容表示注释内容。 这是不会被执行的语句。 如果要注释多行， 需要写到#=...=#之间。
- 通常，语句后面如果**没有分号；**则该语句执行的结果会显示出来， 如果有分号，则结果不会被显示出来。
- Julia引入了一些Matlab的特性， 如果你输入`ans`， 会自动打印出上一个语句的执行结果
