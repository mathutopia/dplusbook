...
# ╔═╡ 32cd5890-d813-11ed-10dd-c5f49f8ba0c8
using Clustering, Distances,CSV, DataFrames,PlutoUI,StatsPlots

# ╔═╡ cd3a06c5-0ba1-4d98-8ce3-263d2a99cb9b
using DataFramesMeta

# ╔═╡ d750935b-b69a-42b5-a38d-00764c4ff246
using StatsBase

# ╔═╡ cdac2ec7-a6cf-4972-8cc0-811438e2ef7b
using MLBase

# ╔═╡ 8a969216-377a-433c-9425-9bf3e217d81a
TableOfContents(title = "目录")

# ╔═╡ c735acc3-bb62-4467-9e17-b2c7f2995262
md"""
# Hclust
...
# ╔═╡ d667eab6-152d-43be-8170-89bf620e0a62
md"""
## 距离计算
...
# ╔═╡ b335e906-3165-49bf-826a-24751ac2c40b
md"""
下面以竞赛数据集为例做演示， 为了方便，采用跟上一节课同样的数据处理方式。
"""

# ╔═╡ beedc56a-0a4b-41b9-a3a9-ac33e3d974f6
train = CSV.read("data/trainbx.csv", DataFrame)

# ╔═╡ 76f1fcdd-f412-4dcc-8d13-4405a84eca2b
...
# ╔═╡ fcbc8686-a739-4e72-878f-4466855219ff
md"""
注意， 下面我们还是类似做了转置， 因为在计算距离时，是按列计算的， 我们计算的目标是样本之间的距离。 此外，我们需要每个样本两两之间的距离， 所以用pairwise的方式计算。
"""

# ╔═╡ 55fbb99f-4265-4215-a442-f0c0dd34e823
X = Matrix(data1[:, 1:4])'

# ╔═╡ 1ce576b0-3596-435e-9f1c-a13e07ee4990
d = pairwise(Euclidean(), X, X)

# ╔═╡ b42e20bb-d5e1-4565-a9b4-97f6600668da
md"""
## 层次聚类
"""

# ╔═╡ 2eba561e-9847-48ae-b498-7e85a79f94a6
h = hclust(d)

# ╔═╡ 147a3bd4-9df4-44cc-8ff5-acb42c35672e
plot(h)

# ╔═╡ 41a6f538-0750-4b83-bd15-982f3692a8eb
md"""
上面将所有的样本画在一起， 可能看不清做种结果长啥样， 我们可以少画一点。 如下为 我们将前50个样本做层次聚类的结果。
"""

# ╔═╡ 08cacdda-0502-4819-948c-3136213a2afc
h1 = hclust(d[1:50, 1:50])

# ╔═╡ 9101fd52-59c7-419b-84a5-a325621e9e5d
plot(h1)

# ╔═╡ 41d8b066-d1da-4ecf-8d35-cfdc409e7558
md"""
## 结果获取
由于层次聚类是将所有的样本汇聚到了一个簇， 可以使用cut函数将其分为不同的簇。
"""

# ╔═╡ 3ca8521f-6900-4a09-a203-8fea180d4993
c = cutree(h, k = 2)

# ╔═╡ e010e23d-d6e4-4580-81b7-edc42f36e527
md"""
采用StatsBase包中的countmap函数可以获取每个簇中有多少样本
"""

# ╔═╡ 801810a9-047f-4ff8-948d-4531dde00385
countmap(c)

# ╔═╡ 9464acea-6a33-4a3b-acab-20e4f46194e8
md"""
## 测试其他参数
在层次聚类中， 如果想改变样本之间的距离计算， 需要自己手动改变， 因为需要自己先计算距离矩阵。但可以调整计算簇之间的距离的方式， 也就是linkage参数。
"""

# ╔═╡ c07f073e-5644-4f74-9cbd-6d0cbdb9a3cc
h2 = hclust(d,  linkage = :average)

# ╔═╡ d476dd4d-8430-48b7-ad26-2ddaf0cd2c21
c2 = cutree(h2, k=2)

# ╔═╡ 8aa22e1c-3baa-4de9-a267-11287cc7a769
md"""
## 查看聚类效果
"""

# ╔═╡ c9fec1e8-bcd3-4c32-bddc-595af69837a3
pred = replace(c, 1 => 1, 2 => 0)

# ╔═╡ e13f6c3d-9bf6-4353-9bb6-0ceb3c3e5cde
r = MLBase.roc(train.fraud, pred)

# ╔═╡ 24fb7bb7-6a2a-4288-8f20-1ba9d406c482
MLBase.precision(r)

# ╔═╡ f56513e4-c7b8-46c6-8c73-8af4ffd2b0b0
MLBase.recall(r)

# ╔═╡ 109d1098-a021-448a-8830-ca5c9b21fd08
md"""
第二种方法的效果是不是一样？
"""

# ╔═╡ a9e6cfb9-d66e-46a0-9fab-0500731375e0
pred2 = replace(c2, 1 => 1, 2 => 0)

# ╔═╡ 664e4101-3466-44b2-8395-d44c5e409052
r2 = MLBase.roc(train.fraud, pred2)

# ╔═╡ 0ee29372-3065-474b-9906-c187c989ce87
MLBase.precision(r2)

# ╔═╡ 9eb65852-d3a4-483b-b190-0b1d2c06c0ef
MLBase.recall(r2)

# ╔═╡ 4894b3a2-2370-45e5-9230-70d1b2e5238d
md"""
两种方式基本一致， 但比上一次的kmeans聚类效果似乎好一点。
"""

# ╔═╡ a180087a-e0c1-46aa-a1b8-b39588386c19
md"""
# Dbscan
[Dbscan](https://juliastats.org/Clustering.jl/stable/dbscan.html)
Dbsacn的函数签名有两种：
...
#### 第一种签名参数
- D Distance (adjacency) matrix (N*N)
...
# ╔═╡ 7a3e3a37-9932-408a-8ad1-9c9caacb68d4
cc= dbscan(d, 2000, 3)

# ╔═╡ d194c9b4-e65c-4b62-b194-a6d39db65fdd
md"""
# 用聚类方法提交一次比赛成绩
"""

# ╔═╡ b92fdc0d-3b5b-4a0e-9a96-b5c76e116adf
md"""
假定我们找到了一种比较好的聚类方法， 可以用于对数据进行分类。那我们就可以利用这个模型来提交一次比赛成绩。

严格来说， 我们需要用到在训练集上得到的模型结果（每个簇有一个类别预测）， 然后把测试集中样本分类去研究应该放入哪个簇，进而得到相应的类别预测。 这种方式稍微需要写一点代码（比如计算每个测试样本跟簇的距离去划分）。 我们采用一种相对更简单的方式， 将在训练集中使用的同样的参数， 用于测试集中样本的聚类， 最有用同样的方式做类别预测。

假定我们选择上面hclust的第一种方式去做竞赛。
"""

# ╔═╡ 6595ae97-4cb1-42ea-9417-3ee8a44cc772
md"""
## 读取测试数据
"""

# ╔═╡ 078c319e-fd84-4da2-b356-7335a3f63f54
test = CSV.read("data/testbx.csv", DataFrame)

# ╔═╡ 82c9e236-930e-4eee-b4ab-73cf4015d54a
md"""
## 进行特征选择和处理
注意， fraud列是测试集中没有的，需要我们去预测。 但由于提交成绩的时候，需要id列（policy_id	fraud
）， 因此， 我们把这一列的信息提取出来。
"""

# ╔═╡ fe2eacdf-baef-4a26-ab2f-c465bcf6819f
data2 = @chain test begin
@transform :autoage = 2023 .- :auto_year # 车辆年限在原始数据中不存在，需要计算出来
@select :age :premium = :policy_annual_premium :claim = :vehicle_claim :autoage :policy_id
end

# ╔═╡ 23f59c58-b089-4ab8-93a7-25c5b06ccccf
md"""
## 提取特征矩阵/计算距离矩阵
"""

# ╔═╡ 9dce3680-b258-46bb-9610-3c917e75e765
Xtest = Matrix(data2[:, 1:4])'

# ╔═╡ 5b99e3c1-7bc9-410f-8631-b2350748563e
dtest = pairwise(Euclidean(), Xtest,Xtest)

# ╔═╡ f8cec128-255c-4e2f-b0af-6df0cc670781
md"""
## 层次聚类
"""

# ╔═╡ 21c216c7-1c9c-437b-83d9-c12f85cbcd5d
htest = hclust(dtest)

# ╔═╡ ae980a53-4d62-4ee4-b706-4e38b305b258
md"""
## 获取簇标签，并预测编号
"""

# ╔═╡ 85d437bb-521b-4a94-9780-f57497b7c674
ctest = cutree(htest, k = 2)

# ╔═╡ 5502eda6-34a4-4cac-a570-55a6ed82f086
prefraud = replace(ctest, 1 => 1, 2 => 0)

# ╔═╡ f1b17ee2-883f-4142-afd3-55b0d9b1e92f
md"""
## 构造提交结果DataFrame

"""

# ╔═╡ 4490c37d-b300-4c76-a8b1-d28bc8038af3
res = DataFrame(policy_id = data2.policy_id, fraud = prefraud)

# ╔═╡ 5e71fc00-f019-4670-8335-62ef90914e70
md"""
!!! info "注意："
	上面构造DataFrame时， 括号中等号的前面是列的名字， 后面是列的值对应的变量。比如fraud = fraud表示， 在结果中有一列名为fraud， 其值是上面预测编号时保存的变量。

	此外， 由于比赛对提交结果是有要求的， 列的名字不能修改。顺序最好也不改。
"""

# ╔═╡ 9c0a8e2a-7735-45e4-972e-399da1c6bfb5
md"""
## 将结果写入csv文件
"""

# ╔═╡ 16106580-a625-4f09-8eba-a203a949f704
CSV.write("submit.csv", res)

# ╔═╡ 37ddeede-f34c-4d73-a771-bbb60e009bd5
pwd()

# ╔═╡ 21e78d2c-f67b-41da-9bbd-2c7c6928ea81
md"""
## 提交预测
最后，只需要在比赛页面， 将上述生成的CSV文件结果提交即可。
"""
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "c5f5c34d3c22250bdc74729c5c3aa3f8a2efaf06"
